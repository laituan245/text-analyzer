			...
			success: function (data) {
				...
				$.ajax({
					...
					success: function (data) {
						...
						for (var a = 0; a < data_payload.length; a++)
							for (var b = a + 1; b < data_payload.length; b++)
								if (parseFloat(data_payload[a].p) < parseFloat(data_payload[b].p)) {
									var tg = data_payload[a];
									data_payload[a] = data_payload[b];
									data_payload[b] = tg;
								}
						topic_p = [];
						topic_names = [];
						...
						for (var j = 0; j < data_payload.length; j++) 
							if (Math.round(parseFloat(data_payload[j].p) * 100) > 0 ) {
								topic_names.push(data_payload[j].className);
								tmp_booleans_arr.push(false);
								topic_p.push(parseFloat(data_payload[j].p));
								
								var tmpInt = Math.round(parseFloat(data_payload[j].p) * 100);
								var tmpDiv = document.createElement('div')
								var tmpSpan = document.createElement('span');
								tmpDiv.className = 'col-sm-12';
								tmpSpan.innerHTML = data_payload[j].className + ' (About ' + tmpInt + '%)';
								...
								$('#topics-container').append(tmpDiv);
							}
							...
							$.ajax({
								...
								success: function (data) {
									...
									data_payload.forEach(function (subtopicdata) {
										...
										subtopicdata.className = subtopicdata.className.replace('_',' ');
									});
									subtopicsdata = subtopicsdata.concat(data_payload);
									var isAllTrue = true;
									for (var tmpLoopIdx = 0; tmpLoopIdx < tmp_booleans_arr.length; tmpLoopIdx++)
										if (tmp_booleans_arr[tmpLoopIdx] === false)
											isAllTrue = false;
									if (isAllTrue) {
										data_payload = subtopicsdata;
										
										for (var a = 0; a < data_payload.length; a++)
											for (var b = a + 1; b < data_payload.length; b++)
											if (data_payload[a].p < data_payload[b].p) {
												var tg = data_payload[a];
												data_payload[a] = data_payload[b];
												data_payload[b] = tg;
											}
										...
										for (var j = 0; j < data_payload.length; j++) 
											if (Math.round(data_payload[j].p * 100) > 0 ) {
												var tmpInt = Math.round(data_payload[j].p * 100);
												var tmpDiv = document.createElement('div')
												var tmpSpan = document.createElement('span');
												tmpDiv.className = 'col-sm-12';
												tmpSpan.innerHTML = data_payload[j].className + ' (About ' + tmpInt + '%)';
												...
												tmpProgress.appendChild(tmpProgressBar);
												tmpDiv.appendChild(tmpProgress);
												$('#subtopics-container').append(tmpDiv);
											}
